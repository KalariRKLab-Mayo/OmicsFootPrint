...
def report(fish_list1, result_list1,prob1, name1):

    for ind in list(set(fish_list1)):
        name=name1+'_'+str(ind)
        fish_list=[ 1 if i==ind else 0 for i in fish_list1 ]
        result_list=[ 1 if i==ind else 0 for i in result_list1 ]
...
for line in fobj:
    ...
    if line[3] == "train":
        ...
        train_pred.append(int(line[5]))
        train_prob.append(float(line[4]))
    if line[3] == "test":
        test_ori.append(int(line[2]))
        test_pred.append(int(line[5]))
        test_prob.append(float(line[4]))
    if line[3] == "val":
        val_ori.append(int(line[2]))
        val_pred.append(int(line[5]))
        val_prob.append(float(line[4]))
fobj.close()
