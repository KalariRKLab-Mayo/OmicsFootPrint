...
prob_aucs = [roc_auc_score(train_label,[ 1 if x >= softmax_range[i] else 0 for x in train_softmax ]) for i in range(len(softmax_range))]
#print(prob_aucs)
...
train_samples_uniq_label_softmax = [ np.mean([ train_softmax[x] for x in range(len(train_samples)) if train_samples[x] == train_samples_uniq[i] ]) for i in range(len(train_samples_uniq))]
train_samples_uniq_label_pred = [ 1 if i > threshold else 0 for i in train_samples_uniq_label_softmax ]
...
val_samples_uniq_label_softmax = [ np.median([ val_softmax[x] for x in range(len(val_samples)) if val_samples[x] == val_samples_uniq[i] ]) for i in range(len(val_samples_uniq))]
val_samples_uniq_label_pred = [ 1 if i > threshold else 0 for i in val_samples_uniq_label_softmax ]
...
test_samples_uniq_label_softmax = [ np.mean([ test_softmax[x] for x in range(len(test_samples)) if test_samples[x] == test_samples_uniq[i] ]) for i in range(len(test_samples_uniq))]
test_samples_uniq_label_pred = [ 1 if i > threshold else 0 for i in test_samples_uniq_label_softmax ]
