...
class Preprocess:
    ...
    def __init__(self, directory_path, filetype, tfrecord_image, tfrecord_label, loss_function=None,batch_size=None):
        """
        ...
        self.files, self.labels, self.label_dict, self.min_images, self.filetype, self.tfrecord_image, self.tfrecord_label, self.ori_labels = self.__get_lists()
    ...
    def __get_lists(self):
        ...
        if flag==0 :
            ...
            for i in range(0,len(classes),1):
                ...
                if max_len > len(tmp):
                    tmp1 = tmp+[ tmp[z] for z in np.random.randint(len(tmp), size=(max_len-len(tmp))).tolist()]
                else:
                    tmp1 = tmp
        ...
        del files_list
        del labels_list
        #print(len_list,len(labels))
        #sys.exit(0)
        while self.batch_size>len(files):
            files=files+files
            labels=labels+labels
            
        #print(self.batch_size,len(files),len(labels),label_dict)
        #sys.exit(0)
        ori_labels = labels    
        ...
        min_images = len(labels)    
        return files, labels, label_dict, min_images, filetype, tfrecord_image, tfrecord_label,ori_labels
...
def format_example_aug(image_name=None, img_size=256):
    """
    Apply any image preprocessing here
    :param image_name: the specific filename of the image
    :param img_size: size that images should be reshaped to
    ...
    if train is True:
        image = tf.image.random_flip_left_right(image)
        image = tf.image.random_flip_up_down(image)
        image = tf.image.random_brightness(image, max_delta=0.28, seed=44)
        image = tf.image.random_saturation(image, lower=0.28, upper=2.8)
        image = tf.image.random_hue(image, max_delta=0.28)
        image = tf.image.random_contrast(image, lower=0.28, upper=2.8, seed=44)
...
def format_example_albu(image_name=None, img_size=256):
    """
    Apply any image preprocessing here
    :param image_name: the specific filename of the image
    :param img_size: size that images should be reshaped to
    ...
    # if train is True:
        ...
        # image = tf.image.convert_image_dtype(image, dtype=common_type, saturate=False)
