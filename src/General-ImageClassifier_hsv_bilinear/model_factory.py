...
class GetModel:
    ...
    def __get_model_and_preprocess(self, retrain):
        ...
        conv=model.get_layer('block4_conv3') # block4_conv3
        d1=Dropout(0.5)(conv.output)
        d2=Dropout(0.5)(conv.output)
        #d1=conv.output
        #d2=conv.output

        x = Lambda(self.outer_product, name='outer_product')([d1,d2])
        x = Flatten()(x)
        x = Dropout(0.30)(x)
        x = Dense(512, activation='relu', kernel_initializer='he_normal',kernel_regularizer=regularizers.l2(0.001), bias_regularizer=regularizers.l2(0.001))(x)
        #x = Dense(512, activation='relu')(x)
        predictions=Dense(self.classes, activation='softmax', name='predictions')(x)

        model1 = Model(inputs=model.input, outputs=predictions)
        return model1, preprocess
    ...
    def outer_product(self,x):
        ...
        phi_I = tf.math.divide(phi_I, size1*size2)
        
        # Take signed square root of phi_I
        y_ssqrt = tf.math.multiply(tf.math.sign(phi_I),tf.math.sqrt(tf.math.abs(phi_I)+1e-12))
