...
def argument_parse():
    ...
    parser.add_argument("-m", "--input_label_file", help="input input_label_file", required="True")
    return parser
...
def create_binary_mask_new(input_label_file,svs_file,patch_dir,patch_level):			
    ...
    for i in fobj:
        ...
        if float(arr1[5]) > 0.5:
            ...
            patch1 = PolygonPatch(Polygon([(x1, y1), (x2, y1), (x2, y2), (x1, y2), (x1, y1)]), facecolor=[0, 0, 0], edgecolor="red", alpha=tmp, zorder=2)
            ax.add_patch(patch1)
        else:
            tmp = round(float(arr1[5]), 2)
            patch1 = PolygonPatch(Polygon([(x1, y1), (x2, y1), (x2, y2), (x1, y2), (x1, y1)]), facecolor=[0, 0, 0], edgecolor="green", alpha=tmp, zorder=2)
            ax.add_patch(patch1)    
    fobj.close()        
